...
#include <stdlib.h>  // Add this to fix the implicit declaration of 'strtoul'
...
int main (int argc, char *argv[]) {
  ...
  while ((i = getopt(argc, argv, "k:p:")) >= 0) {
    switch (i) {
    case 'k':
    if (strlen(optarg) == 20 && KEYSIZE == 80) {
        for (int i = 0; i < 10; i++) {
            char hexPair[3] = { optarg[i * 2], optarg[i * 2 + 1], '\0' };  
            unsigned char byte = (unsigned char)strtol(hexPair, NULL, 16); 
            x[i] = byte;  
        }
  }   
    else if (strlen(optarg) == 32 && KEYSIZE == 128) {
    ...
    if (strlen(optarg) == 16) {
        ...
        unsigned long long value = strtoull(optarg, NULL, 16); 
        
       
        s.b[0] = (unsigned int)(value >> 32);  
        s.b[1] = (unsigned int)(value & 0xFFFFFFFF); 
    } else {
        printf("Error! Wrong size of plaintext block. Expected 16 hex characters.\n");
        return EXIT_FAILURE;
    }
    break;
